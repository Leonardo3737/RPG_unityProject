...
public class PlayerStateMachine : StateMachine
{
    ...
    public int UnequippedLayer = 1;
    public int EquippedLayer = 2;
    ...
    public void HandleMainAttack()
    {
        ...
        ChangeState(
            new PlayerAttackState(this, AttackTypes.MainAttack)
        );
    }
    ...
    public void HandleAlternativeAttack()
    {
        if (!((PlayerBaseState)currentState).CanPerformAction()) return;
        ChangeState(
            new PlayerAttackState(this, AttackTypes.AlternativeAttack)
        );
    }
    ...
    public void ToggleMode()
    {
        if (CurrentMode == Modes.UNARMED)
        {
            ...
            CurrentEquipament.transform.SetLocalPositionAndRotation(
                new Vector3(0.08f, 0.748f, -0.416f),
                Quaternion.Euler(0, 90, 85.251f)
                );
    ...
    /* public void ToggleAnimatorController()
    {
        ...
        {
            Animator.SetLayerWeight(UnequippedLayer, 0); // Ativa layer "Armed"
            Animator.SetLayerWeight(EquippedLayer, 1); // Desativa
            CurrentLayer = EquippedLayer;
        }
        else
        {
            Animator.SetLayerWeight(EquippedLayer, 0);
            Animator.SetLayerWeight(UnequippedLayer, 1);
            CurrentLayer = UnequippedLayer;
        }
    } */

    public GameObject UnequipBow()
    {
        var currentBow = Instantiate(BowPrefab, BowHolder);
        currentBow.transform.localPosition = new Vector3(0.08f, 0.748f, -0.416f);
        currentBow.transform.localRotation = Quaternion.Euler(0, 90, 85.251f);
        return currentBow;
    }
